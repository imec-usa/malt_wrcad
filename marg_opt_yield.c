...
#define ADDPOINT_INIT {.returnn = NULL, .call = NULL, .pc = NULL, .pid = 0, .ord = 0}
...
static bool addpoint_is_done(const addpoint_t *state) {
  return state->pid == 0;
}
...
int margins(Configuration *C, const Space *S, double *prhi, double *prlo)
{
  ...
  /* Names & Nominals */
  for (i=0; i < N + K; i++) {
    lprintf(C, "%3d) %-19.19s %8.3f %8.4f %4d  ",
            i+1,
            C->params[i].name,
    ...
    if (C->params[i].corners) {
      lprintf(C, "%8.3f %8.3f\n",
              physspace(S[i].cornerlo,C,i),
              physspace(S[i].cornerhi,C,i));
    } else {
      lprintf(C, "       0\n");
    }
  }
  ...
  for (i=0; N > i; ++i) {
    ...
    /* the sigmas */
    lprintf(C, "     %7.2f%s %7.2f%s",
            (prlo[i] - S[i].centerpnt),
            (prlo[i] <= C->params[i].min)?"*":" ",
            (prhi[i] - S[i].centerpnt),
...
int tmargins(Configuration *C, const Space *S)
{
  ...
  for (i=0; N > i; ++i) {
    ...
    if (C->params[i].top_max)
      fprintf(fp,"load %s.%s.max.pass\nload %s.%s.max.fail\n",
              C->command,C->params[i].name,C->command,C->params[i].name);
    /* lower margin */
    if (C->params[i].top_min)
  ...
  for (k=3, i=0; N > i; ++i) {
    for (j=0; C->num_nodes > j; ++j) {
      ...
      fprintf(fp," \\\ntran%d.%s \\\ntran%d.%s",
              k,C->nodes[j].name,k+1,C->nodes[j].name);
      /* both lower and upper margin */
      if (C->params[i].top_min && C->params[i].top_max) {
        fprintf(fp," \\\ntran%d.%s \\\ntran%d.%s",
                k+2,C->nodes[j].name,k+3,C->nodes[j].name);}
    }
    k +=2;
    if (C->params[i].top_min && C->params[i].top_max)  k +=2;
    fprintf(fp,"\n");
  }
  fprintf(fp,"endif\n\n.endc\n");
  fclose(fp);
...
double det_dim(double **aa, int n)
{
  ...
  for (i=1;i<=n;i++) {
    big=0.0;
    for (j=1;j<=n;j++)
      if ((tem=fabs(aa[i][j])) > big) big=tem;
    if (big == 0.0)  {nrerror("in routine det_dim");}
    vv[i]=1.0/big;}
  for (j=1;j<=n;j++) {
    for (i=1;i<j;i++) {
      sum=aa[i][j];
      for (k=1;k<i;k++) sum -= aa[i][k]*aa[k][j];
      aa[i][j]=sum;}
    big=0.0;
    for (i=j;i<=n;i++) {
      sum=aa[i][j];
...
int simplx(double *const *a,int m,int n,int *izrov,int *iposv)
{
  ...
  nl2=m;
  for (i=1;i<=m;i++) {
    if (a[i+1][1] < 0.0) nrerror("in routine simplx");
    l2[i]=i;
    iposv[i]=n+i;}
/* stage one */
  ir=1;
